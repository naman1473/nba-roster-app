...
/**
 * Using Aadya Prasad's RBT Implementation for Integration Week
 *
 * @author Aadya Prasad
 */
...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    /**
     * This recursive method fixes any red black tree properties that were violated
     * after the insertion of the new node. The method is called within the insert method.
    ...
    protected void enforceRBTreePropertiesAfterInsert(Node<T> node) {
        ...
        if (parent.isRightChild()) {
            uncle = parent.context[0].context[1];
        } else {
            uncle = parent.context[0].context[2];
        }
        ...
        // Case 4: parent is red, but uncle is black or null and the new node and parent are not on the same side
        ...
        // Case 5: parent is red, but uncle is black or null and the new node and parent are on the same side
        parent.blackHeight = 1;
        grandparent.blackHeight = 0;
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (child == null || parent == null) {
            throw new IllegalArgumentException("Both child and parent nodes must be provided.");
        }
        ...
        if (parent.context[1] == child) {
            ...
            // check if parent is the root of tree
            if (grandParent == null) {
                root = child;
                child.context[0] = null;
            }

            // for when parent is not root of tree
            if (grandParent != null) {
            ...
            throw new IllegalArgumentException("Provided child and parent nodes are not related.");
        }
